...
mocks:  ## Start server in mock mode (usage: make mocks [env=production|acceptance])
	@if [ -z "$(env)" ]; then \
		read -p "Which environment? [production/acceptance]: " env; \
	fi; \
	if [ "$(env)" = "production" ]; then \
		PP_DBN=EuroTourno ./scripts/start-server.sh 4000 mobile true EuroTourno; \
	elif [ "$(env)" = "acceptance" ]; then \
		PP_DBN=AccTourno ./scripts/start-server.sh 4010 mobile true AccTourno; \
	else \
