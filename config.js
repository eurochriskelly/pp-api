...
if (process.env['PP_DBN'] !== 'MockTourno') {
  if (!process.env['PP_HST']) {
    console.log(`Env PP_HST is not defined for non-mock DB [${process.env['PP_DBN']}] .. exiting.`);
    process.exit(1);
  }
  if (!process.env['PP_USR']) {
    console.log(`Env PP_USR is not defined for non-mock DB [${process.env['PP_DBN']}] .. exiting.`);
    process.exit(1);
  }
  if (!process.env['PP_PWD']) {
    console.log(`Env PP_PWD is not defined for non-mock DB [${process.env['PP_DBN']}] .. exiting.`);
    process.exit(1);
  }
}
