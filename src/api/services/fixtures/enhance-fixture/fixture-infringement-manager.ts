...
export interface FixtureInfringementManagerDependencies {
...
export class FixtureInfringementManager {
  private select: (sql: string, params: any[]) => Promise<any[]>;
  private logger: (msg: string, ...args: any[]) => void;

  constructor({ select, logger }: FixtureInfringementManagerDependencies) {
    this.select = select;
    this.logger = logger;
  }

  isPlaceholderTeam(name: string | undefined | null): boolean {
    return (
      typeof name === 'string' &&
  ...
  async processTeamInfringements(
    fixture: Fixture,
    teamName: string,
    teamInfringementArray: Infringement[]
  ): Promise<void> {
    ...
    expelledPlayers.forEach((p: PlayerCard) => {
      this.logger(
...
`;
    ...
    const counts = cards.reduce(
      (
        acc: Record<string, CardCount>,
        { playerNumber, playerName }: CardInfo
      ) => {
    ...
    Object.values(counts).forEach((p: CardCount) => {
      if (p.count >= 2) {
...
export default FixtureInfringementManager;
