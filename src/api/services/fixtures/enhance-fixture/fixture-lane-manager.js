...
class FixtureLaneManager {
    ...
    if (!fixture?.started) {
      if (fixture.pitch && fixture.tournamentId && typeof fixture.id !== 'undefined') {
        ...
        if (nextUnstartedOnPitch.length > 0 && nextUnstartedOnPitch[0].id === fixture.id) {
          this.logger(`Fixture [${fixture.id}] is the next unstarted on pitch [${fixture.pitch}]. Lane: 'queued'.`);
          return 'queued';
        }
        this.logger(`Fixture [${fixture.id}] is planned but not the next on pitch [${fixture.pitch}]. Lane: 'planned'.`);
        return 'planned';
      }
      this.logger(`Fixture [${fixture.id}] missing pitch, tournamentId, or id for 'queued' check. Defaulting to 'planned'.`);
      return 'planned';
    }
  ...
  async getAllowedLanes(fixture, currentLane) {
    ...
    if (team1Identifier.startsWith('~') || team2Identifier.startsWith('~')) {
      this.logger(`Fixture [${fixture.id}] (Tournament: ${fixture.tournamentId}): Team identifier (${team1Identifier} or ${team2Identifier}) starts with '~'. Allowed lanes restricted to ['planned'].`);
      return ['planned'];
    }
    ...
    if (fixture.pitch && fixture.tournamentId && typeof fixture.id !== 'undefined') {
      ...
      if (ongoingFixturesOnPitch.length > 0) {
        this.logger(`Fixture [${fixture.id}] (Tournament: ${fixture.tournamentId}, Pitch: ${fixture.pitch}): Another match on the same pitch is 'started'. Removing 'started' from allowed lanes for this fixture.`);
      ...
      this.logger(`Fixture (ID: ${fixture.id}, Tournament: ${fixture.tournamentId}, Pitch: ${fixture.pitch}): Skipping Pitch Occupancy check for allowedLanes due to missing field(s): ${missingFields.join(', ')}.`);
    }

    if (currentLane === 'planned') {
      this.logger(`Fixture [${fixture.id}] (Tournament: ${fixture.tournamentId}): Current lane is 'planned'. Removing 'started' from allowed lanes as it must be 'queued' first.`);
      finalAllowedLanes = finalAllowedLanes.filter(lane => lane !== 'started');
    }
