...
export interface FixtureLaneManagerDependencies {
...
export type Lane = 'planned' | 'queued' | 'started' | 'finished';

export class FixtureLaneManager {
  private select: (sql: string, params: any[]) => Promise<any[]>;
  private logger: (msg: string) => void;

  constructor({ select, logger }: FixtureLaneManagerDependencies) {
    this.select = select;
    this.logger = logger;
  }

  async getCurrentLane(fixture: Fixture): Promise<Lane> {
    if (!fixture?.started) {
      if (
        fixture.pitch &&
  ...
  async getAllowedLanes(fixture: Fixture, currentLane: Lane): Promise<Lane[]> {
    const defaultAllowed: Lane[] = ['planned', 'queued', 'started', 'finished'];
    ...
    let finalAllowedLanes: Lane[] = [...defaultAllowed];
    if (
      fixture.pitch &&
      fixture.tournamentId &&
      typeof fixture.id !== 'undefined'
...
export default FixtureLaneManager;
