...
function calculateMockLifecycleStatus(
  ...
  if (dbStatus === 'past') {
    // Assuming mock 'past' means it's effectively closed for this calculation
...
module.exports = () => {
  ...
  } = require('../tournaments/generate-fixtures.js');
  ...
  return {
    ...
    getTournaments: async () => {
      // statusQuery is the original DB status filter, not lifecycle
    ...
    getTournamentsByStatus: async (requestedStatusString, userId, region) => {
      ...
      if (requestedStatuses.length > 0) {
        filteredTournaments = filteredTournaments.filter((t) => {
          ...
          let filterLifecycleStatus = calculateMockLifecycleStatus(
            t.status,
            t.startDate,
            t.endDate
          );
      ...
      return filteredTournaments.map((t, index) => {
        ...
        return {
          ...
          lifecycleStatus: calculateMockLifecycleStatus(
            ...
            t.startDate,
            t.endDate
          ), // Use the canonical mock calculator
    ...
    getTournamentsSummary: async () => {
      ...
      mockTournaments.forEach((t) => {
        ...
        if (dbStatus === 'past') {
          ...
          // Fallback if mock status is not perfectly set
          derivedStatus = 'upcoming';
        } else {
