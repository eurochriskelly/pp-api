...
export interface Competition {
  ...
  code: string;
  preliminaryStage?: PreliminaryStage;
...
function getPositionText(position: number | string): string {
  ...
  const posNum = parseInt(position as string, 10);
  if (isNaN(posNum)) return position as string;
...
function getDisplayTeamName(
  teamId: string,
  competitionCode: string = 'M'
): string {
  if (!teamId || !teamId.startsWith('~')) {
    return teamId;
  }

  const winnerLoser = (pos: string) => (pos === '1' ? 'WINNER' : 'LOSER');
 ...
 */
function generateGroupFixtures(teamIds: string[], duration: number): Fixture[] {
  ...
  if (teams.length % 2 !== 0) {
    teams.push(BYE as unknown as string);
  }
 ...
 */
function generateBracketFixtures(
  bracket: Bracket,
  duration: number,
  ...
  if (numberOfTeams >= 5 && numberOfTeams <= 8) {
    ...
    const participants = [...teams, ...byes].sort();
 ...
 */
function generateFixturesForCompetition(competition: Competition): Competition {
...
export { generateFixturesForCompetition };
export default generateFixturesForCompetition;
