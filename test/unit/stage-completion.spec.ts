    ...
    dbHelpers: {
      select: async (sql: string, params: any[]) => {
        ...
        if (sql.includes('FROM (STANDINGS_H2H) AS h2h_data')) {
          return [
            { team: 'A', grp: 1, position: 1, TotalPoints: 3, PointsDifference: 5, PointsFrom: 10 },
            { team: 'B', grp: 1, position: 2, TotalPoints: 0, PointsDifference: -5, PointsFrom: 4 },
          ];
        }

        if (sql.includes('SELECT team1Id AS team1, team2Id AS team2')) {
          return [
            { team1: 'A', team2: 'B', goals1: 1, points1: 2, goals2: 0, points2: 1, outcome: 'played' },
            { team1: 'A', team2: 'C', goals1: null, points1: null, goals2: null, points2: null, outcome: 'not played' },
          ];
        }
